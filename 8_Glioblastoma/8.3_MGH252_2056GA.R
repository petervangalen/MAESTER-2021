...
# Visualize informative variants in glioblastoma sample
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
# Visualize 2056_G>A for Supplement #
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
...
g1 <- metadata.tib %>%
    ...
    scale_color_manual(values = celltype.col[names(celltype.col) %in% metadata.tib$CellType]) +
...
g2 <- metadata.tib %>%
    ...
    geom_point_rast(size = 0.5) +
    scale_color_manual(values = subset.col[names(subset.col) %in% metadata.tib$Site]) +
    guides(colour = guide_legend(override.aes = list(size = 3))) +
    theme_bw() +
    theme(aspect.ratio = 1, panel.grid = element_blank())
...
pdf("8.3_Plots_2056.pdf", width = 15, height = 6)
