...
maegatk_all.rse <- readRDS("../1_MT_Coverage/TenX_CellLineMix_All_mr3_maegatk.rds")
...
vars.tib %>%
    ggplot(aes(x = mean_af.K562, y = mean_af.BT142, color = var %in% voi.ch)) +
...
# Save variants of interest for comparison with bulk ATAC-seq
vars.tib %>% filter(var %in% voi.ch) %>%
    ...
    write_tsv(file = "FigureS7C_vars.txt")
